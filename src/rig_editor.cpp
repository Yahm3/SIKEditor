...
void RigEditor::DrawUI() {
  ...
  // Create a manual window for the dockspace so we can add padding around the
  // edges
  ...
  while (!WindowShouldClose()) {
    ...
    if (!overUI) {
      ...
      if (wheel != 0) {
        ...
        camera.zoom = std::max(0.15f, std::min(4.0f, camera.zoom + wheel * 0.1f));
        Vector2 afterZoom = GetScreenToWorld2D(mouseScreen, camera);
        camera.target = Math::V2Add(camera.target, Math::V2Sub(beforeZoom, afterZoom));
      }
    }
    if (IsMouseButtonDown(MOUSE_MIDDLE_BUTTON)) {
      Vector2 delta = GetMouseDelta();
      camera.target = Math::V2Sub(camera.target, Math::V2Scale(delta, 1.0f / camera.zoom));
    }
